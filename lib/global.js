...
export function GlobalContextProvider(props) {
  ...
  const [articleLockStatus, setArticleLockStatus] = useState({}) // 文章锁定状态管理
  ...
  // 获取文章的锁定状态
  const getArticleLockStatus = postId => {
    if (!postId) return true // 默认锁定
    return articleLockStatus[postId] ?? true
  }

  // 设置文章的锁定状态
  const setArticleLock = (postId, lockStatus) => {
    if (!postId) return

    // 如果状态没有变化，不触发更新
    if (articleLockStatus[postId] === lockStatus) return

    setArticleLockStatus(prev => ({
      ...prev,
      [postId]: lockStatus
    }))
  }
